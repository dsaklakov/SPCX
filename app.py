...
def polygon_snapshot(ticker, api_key):
    ...
    if timestamp:
        snapshot_time = datetime.fromtimestamp(timestamp / 1_000_000_000, tz=timezone.utc).strftime("%Y-%m-%d %H:%M:%S UTC")
    else:
...
def yahoo_snapshot(ticker):
    ...
    price = fast.get("lastPrice") or fast.get("last_price") or float(history.iloc[-1]["Close"])
...
def derive_market_history_inputs(ticker, current_price, manual_price_5d, manual_volatility_ratio, manual_avg_volume_20d):
    history = safe_daily_history(ticker)
    notes = []

    price_5d = float(manual_price_5d)
    volatility_ratio = float(manual_volatility_ratio)
    avg_volume_20d = float(manual_avg_volume_20d)
    ...
    if not history.empty and "Close" in history.columns:
        ...
        if len(closes) >= 5:
            ...
            price_5d = float(closes.iloc[0])
            notes.append("Momentum uses first available close because fewer than 5 trading days are available.")

        returns = np.log(closes / closes.shift(1)).dropna()
        if len(returns) >= 10:
            short_vol = float(returns.tail(10).std())
            if len(returns) >= 90:
                long_vol = float(returns.tail(90).std())
            else:
                long_vol = float(returns.std())
                notes.append("Volatility uses available history because fewer than 90 trading days are available.")
            if long_vol > 0:
                volatility_ratio = short_vol / long_vol
        else:
            notes.append("Volatility uses manual fallback because fewer than 10 daily returns are available.")

    if not history.empty and "Volume" in history.columns and len(history["Volume"].dropna()) >= 1:
        volume_series = history["Volume"].astype(float).dropna()
        avg_volume_20d = float(volume_series.tail(20).mean())
    else:
        notes.append("Average volume uses manual fallback.")

    return price_5d, volatility_ratio, avg_volume_20d, notes


def build_tables(snapshot, targets, risk_weight, position_size, ipo_shares_sold, implied_total_shares, horizon_minutes):
    ...
    for target in targets:
        ...
        probability = probability_to_target(target, price, high, low, equilibrium, snapshot["snapshot_time"], horizon_minutes)
        delta = target - price
        ...
        action = "DOWNSIDE WATCH" if direction == "Downside" else "AT SPOT" if direction == "At spot" else "PLACE LIMIT SELL"
...
def format_model_value(metric, value):
    ...
    if "range %" in metric_lower or "gain" in metric_lower or "risk to stop" in metric_lower or "live vs open" in metric_lower:
        return percent(value)
    if "minutes" in metric_lower:
        return f"{value:,.1f}"
    if any(word in metric_lower for word in ["floor", "equilibrium", "price", "ceiling", "range", "upside", "downside", "point", "delta", "p/l", "stop"]):
        return money(value)
    return f"{value:,.3f}"
...
def format_stat_value(metric, value):
    if isinstance(value, str):
        return value
    metric_lower = metric.lower()
    if "price" in metric_lower or "high" in metric_lower or "low" in metric_lower or metric_lower == "open" or "dollar volume" in metric_lower or "market cap" in metric_lower:
        return money(value)
...
def add_engine_row(rows, block, factor, raw_value, score, weight):
    rows.append(
        {
            "Block": block,
            "Factor": factor,
            ...
            "Score": float(score),
            "Weight": float(weight),
            "Contribution": float(score) * float(weight) / 100.0,
        }
    )


def build_sell_engine_table(inputs):
    current_price = inputs["current_price"]
    fair_value = inputs["fair_value"]
    price_5d = inputs["price_5d"]
    ...
    expected_upside_raw = compute_expected_upside_pct(current_price, inputs["sell_targets"], inputs["sell_probabilities"])
    momentum_raw = ((current_price - price_5d) / price_5d * 100.0) if price_5d > 0 else 0.0
    price_position = ((current_price - inputs["day_low"]) / (inputs["day_high"] - inputs["day_low"])) if inputs["day_high"] > inputs["day_low"] else 0.0
    volume_ratio = inputs["current_volume"] / avg_volume_20d if avg_volume_20d > 0 else 0.0
    position_pct = position_value / total_portfolio_value * 100.0 if total_portfolio_value > 0 else 0.0

    rows = []
    add_engine_row(rows, "Market", "Valuation", f"{valuation_raw:.1f}% premium", score_valuation(current_price, fair_value), 10)
    add_engine_row(rows, "Market", "Expected Upside", f"{expected_upside_raw:.1f}% heuristic EV", score_expected_upside(current_price, inputs["sell_targets"], inputs["sell_probabilities"]), 9)
    add_engine_row(rows, "Market", "Momentum", f"{momentum_raw:.1f}% vs reference", score_momentum(current_price, price_5d), 8)
    add_engine_row(rows, "Market", "Volatility", f"{volatility_ratio:.2f}x 10d/90d", score_volatility_ratio(volatility_ratio), 5)
    add_engine_row(rows, "Market", "Liquidity Climax", f"range pos {price_position:.2f}, volume {volume_ratio:.2f}x", score_liquidity_climax(current_price, inputs["day_low"], inputs["day_high"], inputs["current_volume"], avg_volume_20d), 5)
    add_engine_row(rows, "Portfolio", "Position Size", f"{position_pct:.1f}% of portfolio", score_position_size(position_value, total_portfolio_value), 9)

    sell_engine_df = pd.DataFrame(rows)
    implemented_weight = float(sell_engine_df["Weight"].sum())
    partial_contribution = float(sell_engine_df["Contribution"].sum())
    current_engine_score = partial_contribution / implemented_weight * 100.0 if implemented_weight else 0.0
    market_df = sell_engine_df[sell_engine_df["Block"] == "Market"]
    portfolio_df = sell_engine_df[sell_engine_df["Block"] == "Portfolio"]
    market_score = float(market_df["Contribution"].sum() / market_df["Weight"].sum() * 100.0) if not market_df.empty else 0.0
    portfolio_score = float(portfolio_df["Contribution"].sum() / portfolio_df["Weight"].sum() * 100.0) if not portfolio_df.empty else 0.0
    return sell_engine_df, partial_contribution, implemented_weight, current_engine_score, market_score, portfolio_score
...
st.markdown(
    ...
    <h1 style="font-size:42px; font-weight:700; color:#E5E5E5; margin-bottom:5px;">SPCX LIVE TRADING MODEL</h1>
    <div style="font-size:18px; color:#A0A0A0;">Real-Time Probability - Risk / Reward - Float Analysis</div>
    </div>
    """,
    unsafe_allow_html=True,
)
...
st.markdown(
    ...
    <div class="spcx-hero-card">
        ...
        <p class="spcx-subtitle">Near-live quote, $63 fair-value drawdown reference, sell-target probabilities, P/L ladder, float math and Sell Decision Engine V1.2 in one execution screen.</p>
    </div>
    """,
    unsafe_allow_html=True,
)
...
with st.sidebar:
    ...
    provider = st.selectbox("Quote provider", ["Polygon", "Yahoo/yfinance", "Manual override"], index=default_provider_index)
    ...
    horizon_minutes = st.number_input("Probability horizon minutes", min_value=5.0, max_value=390.0, value=120.0, step=5.0)
    ...
    fair_value_input = st.number_input("Fair value for Sell Engine", min_value=0.01, value=float(FAIR_VALUE_TARGET), step=1.0)
    manual_price_5d = st.number_input("Manual fallback price for momentum", min_value=0.01, value=135.00, step=0.01)
    manual_volatility_ratio = st.number_input("Manual fallback volatility ratio", min_value=0.01, value=1.00, step=0.05)
    manual_avg_volume_20d = st.number_input("Manual fallback avg volume 20d", min_value=1.0, value=315_425_119.0, step=1000.0)
    total_portfolio_value = st.number_input("Total portfolio value", min_value=1.0, value=100000.0, step=1000.0)
    position_value_manual = st.number_input("SPCX position value", min_value=0.0, value=0.0, step=1000.0)
...
model_df, ladder_df, stats_df = build_tables(snapshot, targets, risk_weight, position_size, ipo_shares_sold, implied_total_shares, horizon_minutes)
...
st.caption(f"Quote timestamp: {snapshot['snapshot_time']} | App refreshed: {snapshot['app_refresh_time']}" + (f" | Auto-refresh run: {refresh_count}" if refresh_count is not None else ""))
...
with right:
    ...
    model_display["Value"] = [format_model_value(metric, value) for metric, value in zip(model_display["Metric"], model_display["Value"])]
...
price_5d, volatility_ratio, avg_volume_20d, history_notes = derive_market_history_inputs(
    ticker,
    float(snapshot["price"]),
    float(manual_price_5d),
    float(manual_volatility_ratio),
    float(manual_avg_volume_20d),
)

position_value = float(position_value_manual) if float(position_value_manual) > 0 else float(snapshot["price"]) * float(position_size)
...
sell_engine_df, sell_engine_partial, sell_engine_weight, sell_engine_score, market_score, portfolio_score = build_sell_engine_table(sell_engine_inputs)
...
engine_cols[3].metric("Market Score", f"{market_score:.1f}")
engine_cols[4].metric("Portfolio Score", f"{portfolio_score:.1f}")
engine_cols[5].metric("Action", recommendation_from_score(sell_engine_score))

st.dataframe(
    sell_engine_df.style.format({"Score": "{:.0f}", "Weight": "{:.1f}", "Contribution": "{:.2f}"}),
    use_container_width=True,
    hide_index=True,
)

if history_notes:
    st.caption(" | ".join(history_notes))

st.caption(
    "Sell Engine V1.2 currently implements 6 of 12 factors: Valuation, Expected Upside, Momentum, Volatility, Liquidity Climax and Position Size. Expected Upside is a heuristic based on ladder reach probabilities, not option-implied EV."
)
...
if not fair_value_df.empty:
    ...
    fv_cols[0].metric("Fair value reference", money(FAIR_VALUE_TARGET), f"{fair_value_row['Delta to target %']:.1%} from current")
    fv_cols[1].metric("Downside probability", probability_text(fair_value_row["Probability to target"]))
    fv_cols[2].metric("Downside move / share", money(fair_value_row["Downside move $/sh"]))
...
prob_text = alt.Chart(sell_ladder_df).mark_text(dy=-8, fontSize=13, fontWeight="bold", color="white").encode(
...
risk_reward_df = sell_ladder_df.melt(id_vars=["Target sell price"], value_vars=["Reward $/sh", "Risk to stop $/sh"], var_name="Line", value_name="USD per share")
...
st.caption("Probability is a realized-range first-passage heuristic based on current quote, intraday high/low, elapsed session time, selected horizon and momentum position. It is not option-implied probability and not a guarantee.")

st.subheader("Market stats and float math")
stats_display = stats_df.copy()
stats_display["Value"] = [format_stat_value(metric, value) for metric, value in zip(stats_display["Metric"], stats_display["Value"])]
st.dataframe(stats_display, use_container_width=True, hide_index=True)

excel_bytes = make_excel(snapshot, model_df, ladder_df, stats_df, sell_engine_df)
st.download_button("Download updated Excel model", data=excel_bytes, file_name=f"{ticker.lower()}_live_model.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
